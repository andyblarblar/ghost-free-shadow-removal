...
def build_aggasatt_joint(input, channel=64, vgg_19_path='None'):
    ...
    for layer_id in range(1, 6):
        ...
        input = tf.concat(
            [tf.compat.v1.image.resize_bilinear(vgg19_f, (tf.shape(input)[1], tf.shape(input)[2])) / 255.0, input],
            axis=3)
...
def conv(batch_input, out_channels, stride):
    with tf.compat.v1.variable_scope("conv"):
        ...
        filter = tf.compat.v1.get_variable("filter", [4, 4, in_channels, out_channels], dtype=tf.float32,
