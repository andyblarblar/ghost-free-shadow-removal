...
if is_training:
        ...
        for id in np.random.permutation(num_train):
            ...
            if input_images_ids[id] is None:
                ...
                if magic < ARGS.use_da:  # choose from fake images
                    ...
                    iminput, imtarget, maskgt = parpare_image(current_img_id, (neww, newh), da=True,
                                                              stage=['_M', '_C', '_B'])
                ...
                if running_idx % 500 == 0:
                    ...
                    cv2.imwrite("%s/%s_%s.png" % (task, running_idx, fileid),
    ...
    for val_path in prepare_data([ARGS.data_dir], stage=[stage]):
        ...
        cv2.imwrite("./results/%s/%s.png" % (subtask + stage, os.path.splitext(os.path.basename(val_path))[0]),
                    imoutput)  # output transmission layer
